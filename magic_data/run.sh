...
# corpus directory
data=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/corpora/mandarin/Magic_Chinese_Mandarin_Conversational_Speech/md_cmn_conversational_speech/
...
# Data Preparation: generate text, text_characters, wav.scp, utt2spk, spk2utt
    local/magic_data_prep.sh $data || exit 1;
    local/magic_data_characters_to_phonemes.sh || exit 1;
fi
...
# Lexicon Preparation: build a large lexicon that involves words in both the training and decoding
   local/magicdata_prepare_dict.sh || exit 1;

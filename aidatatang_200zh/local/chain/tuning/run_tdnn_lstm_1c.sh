...
remove_egs=false
common_egs_dir=exp/chain/tdnn_lstm1c_sp/egs/
...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
  ...
  fast-lstmp-layer name=lstm2 cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-proportion=0.0 $lstm_opts
  relu-batchnorm-layer name=tdnn9 $opts input=Append(0,3,tdnn6l,tdnn4l) dim=$layer_dim

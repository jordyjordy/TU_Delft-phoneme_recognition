...
remove_egs=false
common_egs_dir=exp/chain/tdnn_blstm1c_sp/egs/
...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
  ...
  relu-batchnorm-layer name=tdnn5 $opts dim=$layer_dim
  ...
  fast-lstmp-layer name=lstm2-forward lstm2l cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-proportion=0.0 $lstm_opts
  fast-lstmp-layer name=lstm2-backward input=lstm2l cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=3 dropout-proportion=0.0 $lstm_opts
  relu-batchnorm-layer name=tdnn9 $opts input=Append(lstm2-forward,lstm2-backward,tdnn6l,tdnn4l) dim=$layer_dim
  ...
  output-layer name=output input=tdnn10  include-log-softmax=false $output_opts

  output-layer name=output-xent input=tdnn10 learning-rate-factor=$learning_rate_factor $output_opts
EOF

...
# Assume that ${model_name}.nnet is in nnetdir
cmvn_opts=`cat $nnetdir/cmvn_opts`;
#bnf_nnet=$nnetdir/final.raw
bnf_nnet=$nnetdir/${model_name}.raw
if [ ! -f $bnf_nnet ] ; then
  if [ ! -f $nnetdir/${model_name}.mdl ]; then
    echo "$0: No such file $bnf_nnet or $nnetdir/${model_name}.mdl";
    exit 1;
  else
    bnf_nnet=$nnetdir/${model_name}.mdl
  fi
fi

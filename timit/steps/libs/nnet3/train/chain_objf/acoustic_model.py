    ...
    for job in range(1, num_jobs+1):
        ...
        cache_io_opts = (("--read-cache={dir}/cache.{iter}".format(dir=dir,
                         ...
                         (" --write-cache={0}/cache.{1}".format(dir, iter + 1)
                          if job == 1 else ""))
        if job > 2:    
            time.sleep(6)   # Delays for 6 seconds if for job id 2,3,4,.... You can also use a float value
        elif job == 2:
            time.sleep(8.5) # larger 
        else:
            time.sleep(1.5) # doesn't need to delay
        #print("This intends to pause 2 sec.")
        ...
        threads.append(thread) #suggestion: add time.sleep(5) one line below
        time.sleep(6)

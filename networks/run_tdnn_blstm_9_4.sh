...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
  ...
  relu-batchnorm-layer name=tdnn8 $opts input=Append(lstm3-forward,lstm3-backward,-3,0,3) dim=$layer_dim
  relu-batchnorm-layer name=tdnn9 $opts input=Append(-3,0,3) dim=$layer_dim
  
  fast-lstmp-layer name=lstm4-forward input=tdnn9 cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-proportion=0.0 $lstm_opts
  fast-lstmp-layer name=lstm4-backward input=tdnn9 cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=3 dropout-proportion=0.0 $lstm_opts
  

  output-layer name=output input=Append(lstm4-forward,lstm4-backward) include-log-softmax=false $output_opts

  output-layer name=output-xent input=Append(lstm4-forward,lstm4-backward) learning-rate-factor=$learning_rate_factor $output_opts
EOF

...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
 ...
 opts="l2-regularize=0.00005"
  ...
  relu-batchnorm-layer name=tdnn8 $opts input=Append(-3,0,3) dim=$layer_dim
  relu-batchnorm-layer name=tdnn9 $opts input=Append(-3,0,3) dim=$layer_dim
  norm-opgru-layer name=opgru4 $opts cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-per-frame=true dropout-proportion=0.0

  ## adding the layers for chain branch
  output-layer name=output input=opgru4 include-log-softmax=false $output_opts
  ...
  output-layer name=output-xent input=opgru4 learning-rate-factor=$learning_rate_factor $output_opts

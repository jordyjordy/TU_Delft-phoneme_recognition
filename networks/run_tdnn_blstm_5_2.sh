...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
  ...
  output-layer name=output input=Append(lstm2-forward,lstm2-backward) include-log-softmax=false $output_opts

  output-layer name=output-xent input=Append(lstm2-forward,lstm2-backward) learning-rate-factor=$learning_rate_factor $output_opts
EOF

...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
  ...
  output-layer name=output input=opgru1 include-log-softmax=false $output_opts
  ...
  output-layer name=output-xent input=opgru1 learning-rate-factor=$learning_rate_factor $output_opts
